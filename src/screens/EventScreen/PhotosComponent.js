...
// const imgDummy = 'https://seda.college/wp-content/uploads/party.jpg'

const STATIC_EVENT = '0D9FLM0jlZb12PglGWZc'
...
class PhotosComponent extends Component {
    ...
    switch (type) {
      case 'Member':
        ...
        for (let ind in albums) {
          let photos = _.sortBy(albums[ind], 'datetime');
  ...
  componentDidMount = async () => {
    let response = await ApiManager.getFiles(STATIC_EVENT)

    AsyncStorage.getItem(Const.LOGGED_IN_USER_DETAILS).then((res) => {
      console.log(res)
      debugger
    })

    let arrTempImg = []
    arrTempImg = response.filter(item => item.isMemory == false)
  ...
  uploadImage = async (uri) => {
    ...
    const ref = firebase.storage().ref().child('images/' + 'imageName2')
  ...
  createFileAPICalled = async (url) => {
    const data = {
      "fileName": url,
      "createdByUserId": "EGDtaROorlajrAOb9dmv",
      "createdDateTime": moment(new Date()).format("YYYY-MM-DD"),
      "isMemory": false
    }
    let response = await ApiManager.postCreateFile(STATIC_EVENT, data)
